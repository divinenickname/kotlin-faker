...
internal class FakerService @JvmOverloads internal constructor(
    ...
    private fun load(locale: String): Dictionary {
        ...
        getDefaultFileStreams().forEach {
            readEntry(it, "en").entries.forEach { category ->
...
//             todo Add `separator` category from `locales/en.yml` file
            ...
            readEntry(enYml, "en").entries.forEach { category ->
        ...
        if (locale != "en") {
            ...
            readEntry(localeFileStream, localeString).forEach { cat ->
                when (cat.key) {
    ...
    private fun readEntry(inputStream: InputStream, locale: String): LinkedHashMap<String, Map<String, *>> {
